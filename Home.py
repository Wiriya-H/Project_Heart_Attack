...
s2 = st.selectbox("เพศของผู้ป่วย (0 : หญิง | 1 : ชาย)",[0,1])
s3 = st.selectbox("อาการเจ็บหน้าอก\n(1 : โรคหลอดเลือดหัวใจตีบทั่วไป | 2 : โรคหลอดเลือดหัวใจตีบผิดปรกติ | 3 : อาการปวดที่ไม่ใช่โรคหลอดเลือดหัวใจตีบ | 4 : ไม่มีอาการ)",[1,2,3,4])
s4 = st.number_input("ความดันโลหิตขณะพัก (มม. ปรอท)")
s5 = st.number_input("cholestoral ใน mg/dl ดึงผ่านเซ็นเซอร์ BMI")
s6 = st.selectbox("น้ําตาลในเลือดขณะอดอาหาร > 120 มก./ดล. (1 = จริง 0 = เท็จ)",[0,1])
s7 = st.selectbox("พักผลการตรวจคลื่นไฟฟ้าหัวใจ ( 0 : ปกติ | 1 : มีความผิดปกติของคลื่น ST-T | 2 : แสดงกระเป๋าหน้าท้องยั่วยวนซ้ายที่เป็นไปได้หรือแน่นอนตามเกณฑ์ของเอสเตส)",[0,1,2])
s8 = st.number_input("อัตราการเต้นของหัวใจสูงสุดทําได้")
s9 = st.selectbox("การออกกําลังกายทําให้เกิดโรคหลอดเลือดหัวใจตีบ (1 = ใช่ 0 = ไม่ใช่)",[0,1])
s10 = st.number_input("จุดสูงสุดก่อนหน้า")
s11 = st.selectbox("Slope",[0,1,2])
s12 = st.selectbox("Number of major vessels (0-3)", [0, 1, 2, 3])
s13 = st.selectbox("Thal rate",[0, 1, 2, 3])
